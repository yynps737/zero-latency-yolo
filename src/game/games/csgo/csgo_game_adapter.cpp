#include "csgo_game_adapter.h"

namespace zero_latency {

CSGOGameAdapter::CSGOGameAdapter() : GameAdapterBase(), aim_target_offset_y_(-0.15f), head_size_factor_(0.7f) {
    random_generator_ = std::mt19937(std::random_device{}());
}

Result<void> CSGOGameAdapter::initialize(const GameAdaptersConfig& config) {
    auto result = GameAdapterBase::initialize(config);
    if (result.hasError()) {
        return result;
    }
    
    auto it = config.games.find("csgo");
    if (it == config.games.end() || !it->second.enabled) {
        return Result<void>::error(ErrorCode::INVALID_ARGUMENT, "CSGO game adapter configuration not found or disabled");
    }
    
    const auto& csgo_config = it->second;
    
    aim_target_offset_y_ = csgo_config.aim_target_offset_y;
    head_size_factor_ = csgo_config.head_size_factor;
    
    loadWeaponData(csgo_config.weapons);
    
    LOG_INFO("CSGOGameAdapter initialized successfully");
    
    Event event(events::SYSTEM_STARTUP);
    event.setSource("CSGOGameAdapter");
    publishEvent(event);
    
    return Result<void>::ok();
}

Result<GameState> CSGOGameAdapter::processDetections(uint32_t clientId, const GameState& rawState, uint8_t gameId) {
    if (!initialized_) {
        return Result<GameState>::error(ErrorCode::NOT_INITIALIZED, "Game adapter not initialized");
    }
    
    if (gameId != static_cast<uint8_t>(GameType::CSGO)) {
        return Result<GameState>::error(ErrorCode::INVALID_ARGUMENT, "Unsupported game ID for CSGOGameAdapter");
    }
    ...
    GameState processed_state = processCSGODetections(rawState);
...
Result<int> CSGOGameAdapter::calculateBestTarget(const std::vector<Detection>& detections) {
    if (!initialized_) {
...
Result<BoundingBox> CSGOGameAdapter::predictMotion(const Detection& detection, uint64_t currentTime, uint64_t targetTime) {
    if (!initialized_) {
...
Result<Point2D> CSGOGameAdapter::getAimPoint(const Detection& detection, int weaponId) {
    if (!initialized_) {
...
Result<Vector2D> CSGOGameAdapter::calculateRecoilCompensation(int weaponId, int shotCount, uint64_t timeDelta) {
    if (!initialized_) {
...
std::shared_ptr<IWeaponInfo> CSGOGameAdapter::getWeaponInfo(int weaponId) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<std::string> CSGOGameAdapter::getSupportedGames() const {
    return {"csgo"};
}

std::unordered_map<std::string, std::string> CSGOGameAdapter::getStatus() const {
    auto status = GameAdapterBase::getStatus();
    
    status["game"] = "Counter-Strike: Global Offensive";
    status["weapon_count"] = std::to_string(weapons_.size());
    
    return status;
}

GameState CSGOGameAdapter::processCSGODetections(const GameState& rawState) {
...
void CSGOGameAdapter::loadWeaponData(const std::unordered_map<std::string, GameAdaptersConfig::WeaponConfig>& weaponConfigs) {
    weapons_.clear();
    
    weapons_[1] = std::make_shared<WeaponInfo>(
        1, "AK47", 2.5f, 36.0f, 0.1f, true, 1.0f
    );
    
    weapons_[2] = std::make_shared<WeaponInfo>(
        2, "M4A1", 2.0f, 33.0f, 0.09f, true, 1.0f
    );
    
    weapons_[3] = std::make_shared<WeaponInfo>(
        3, "AWP", 0.0f, 115.0f, 1.5f, false, 1.5f
    );
    
    weapons_[4] = std::make_shared<WeaponInfo>(
        4, "Deagle", 3.0f, 54.0f, 0.4f, false, 1.2f
    );
    ...
    LOG_INFO("Loaded " + std::to_string(weapons_.size()) + " weapons for CSGOGameAdapter");
}

REGISTER_GAME_ADAPTER(CSGOGameAdapterFactory)
