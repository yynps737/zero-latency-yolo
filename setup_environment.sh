...
# 修改 yolo_engine.h 中的头文件包含方式 - 使用更直接的方式
echo -e "${BLUE}检查 yolo_engine.h 文件...${NC}"
YOLO_ENGINE_H="$PROJECT_ROOT/src/server/yolo_engine.h"
if [ -f "$YOLO_ENGINE_H" ]; then
    echo -e "${YELLOW}修复 yolo_engine.h 中的头文件包含...${NC}"
    # 备份原文件
    cp "$YOLO_ENGINE_H" "${YOLO_ENGINE_H}.bak"
    
    # 直接创建修复版本的 yolo_engine.h
    cat > "$YOLO_ENGINE_H" << 'EOF'
